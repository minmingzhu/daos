...
dtx_handle_init(struct dtx_id *dti, daos_handle_t coh,
		...
		int dti_cos_cnt, struct dtx_memberships *mbs,
...
init:
	...
	D_DEBUG(DB_IO, "Start DTX "DF_DTI" ver %u, dti_cos_cnt %d\n",
	  ...
	  struct dtx_memberships *mbs, struct dtx_handle *dth)
{
	...
	dtx_handle_init(dti, cont->sc_hdl, epoch, epoch_uncertain, pm_ver,
			leader_oid, dti_cos, dti_cos_cnt, mbs,
			false, false, dth);

	D_DEBUG(DB_IO, "Start the DTX "DF_DTI" ver %u, dti_cos_cnt %d\n",
		DP_DTI(dti), dth->dth_ver, dti_cos_cnt);
