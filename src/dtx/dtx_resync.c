...
dtx_iter_cb(uuid_t co_uuid, vos_iter_entry_t *ent, void *args)
{
	...
	D_ASSERT(ent->ie_dtx_mbs_ds > 0);
	D_ASSERT(ent->ie_dtx_tgt_cnt > 0);

	size = sizeof(*dre) + sizeof(*mbs) + ent->ie_dtx_mbs_ds;
	D_ALLOC(dre, size);
	if (dre == NULL)
		return -DER_NOMEM;
	...
	mbs->dm_data_size = ent->ie_dtx_mbs_ds;
	memcpy(mbs->dm_data, ent->ie_dtx_mbs, ent->ie_dtx_mbs_ds);
